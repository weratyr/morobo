...
public class MatrixCreator implements IMatrixCreator
	{
		...
		public double getAngleToObstacle(Position mypos,Position scanpos)
			{	System.out.println("scanpos"+scanpos.getX()+","+scanpos.getY());
				//double x=fd.getParsedInfos().getPos()[0];
				//double y=fd.getParsedInfos().getPos()[1];
				double beta;
				double ax = 0, ay = 10; // vektor in lŠngsachse des fahrzeugs
				double lengthveca = Math.sqrt(Math.pow(ax, 2) + Math.pow(ay, 2));
				double lengthvecb = Math.sqrt(Math.pow(scanpos.getX()-mypos.getX(), 2)) + Math.pow(scanpos.getY()-mypos.getY(), 2);
